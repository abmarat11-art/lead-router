...
### Компания уже ведётся в Аргусе

Отдельный случай. Контур перед заведением ищет компанию по ИНН. Если она в Аргусе уже
есть и ведёт её не наш получатель — карточку не трогаем: ответственного не переписываем,
компанию ни у кого не отбираем. Строка закрывается как фрод, ход команде возвращается
долгом, а руководителю команды того, кто ведёт компанию, приходит сообщение в телеграм:
пришёл лид по вашей компании, проверьте, почему она попала в лидогенерацию.

В «Отказы» команды такая строка не попадает — она ничего не отклоняла.
