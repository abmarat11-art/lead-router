...
export async function flushSheetWrites(db, { limit = 20, write, column } = {}) {
  const target = column || (await import('./columns.js')).getConfig().statusColumn;
  ...
  for (const row of rows) {
    ...
    try {
      await writer({ sourceKey: row.source_key, value: row.value, column: target });
