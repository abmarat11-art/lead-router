...
test('строка с id компании Б24 ждёт создания компании и в очередь не идёт', () => {
  const db = setup();
  load(db, rowWithB24());
...
test('карточка из Б24 создаёт компанию в Аргусе, дальше идёт обычное назначение', async () => {
  const db = setup();
  load(db, rowWithB24());
  ...
  const res = await enrichPending(db, {
    ...
    createCompany: async (c) => { created.push(c); return 'argus-900'; },
  });
  assert.deepEqual(res, { picked: 1, ready: 1, failed: 0 });
  assert.equal(created[0].inn, '301234567');
  assert.equal(created[0].contacts[0].phone, '+998901234567');
