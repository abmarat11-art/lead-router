...
test('отказ даёт команде долг: 1,2,3 → отказ у 1 → 1,4,1,2,3', () => {
  const db = setup();
  ...
  markDeclined(db, first, 'отказ');           // ход команды 1 ушёл впустую
...
test('несколько долгов гасятся в порядке поступления', () => {
  ...
  markDeclined(db, b, 'отказ');               // долг: 2
  markDeclined(db, a, 'отказ');               // долг: 2, затем 1
  assert.deepEqual(Array.from({ length: 5 }, () => assign(db)), [2, 1, 3, 4, 1]);
});

test('долг привязан к своей очереди: отказ по лиду не двигает встречи', () => {
  const db = setup();
...
test('отказ закрывает компанию: повторно она не раздаётся', () => {
  const db = setup(2);
  const id = addLead(db);
  assignNext(db, id);                                  // команда 1
  markDeclined(db, id, 'отказ из СРМ');
...
test('погашение долга ставит галочку напротив отказанной строки', () => {
  process.env.SHEETS_SPREADSHEET_ID = 'sheet-id';
  setConfig({ ...DEFAULT_CONFIG, columns: { ...DEFAULT_CONFIG.columns, debt_closed: 'H' } });
  try {
    const db = setup();
    const declined = addLead(db);
    assignNext(db, declined);                        // команда 1
    markDeclined(db, declined, 'отказ');              // долг команде 1
    ...
    assert.equal(debtWrite.lead_id, declined, 'галочка идёт в строку отказа, а не новой компании');
...
test('без колонки чекбокса галочка просто не пишется', () => {
  ...
  try {
    const db = setup();
    const declined = addLead(db);
    assignNext(db, declined);
    markDeclined(db, declined, 'отказ');
